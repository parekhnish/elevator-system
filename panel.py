class Panel(object):
    ...
    def __init__(self,canvas,number,elevator):
        ...
        self.label = self.canvas.create_text(self.PANEL_X + self.PANEL_WIDTH/2,self.PANEL_Y - 15,font="Arial 20 bold", text = "Elevator " + str(self.number))
        ...
        if flag == False:
            ...
            if event == 'G' or event == '<' or event == '>':
                ...
                if self.elevator.current_floor > event:
                    self.elevator.addFloor(event,"down","panel_call")
                else:
